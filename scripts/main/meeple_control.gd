...
func _orders_process():
	#Change this to basically process as many as possible. Idk how, like funny logic
...
func spawn_meeple_order(pos):
	var pixel_position = pos[0]
...
func super_order_order(pos):
	var dest = grid.coord_to_axial_hex(pos[0])
	for m in unorderedMeeples:
		if m.groupNum == 0:
