...
var nexusSpawn = [
	[
		(((MAP_RESOLUTION.x - BORDER_RESOLUTION * 2) / 5) + BORDER_RESOLUTION) * PIXELS_PER_TILE,
		((MAP_RESOLUTION.y - BORDER_RESOLUTION * 2) / 2 + BORDER_RESOLUTION) * PIXELS_PER_TILE
	],
	[
		(
			(((MAP_RESOLUTION.x - BORDER_RESOLUTION * 2) / 5 * 4) + BORDER_RESOLUTION)
			* PIXELS_PER_TILE
		),
		((MAP_RESOLUTION.y - BORDER_RESOLUTION * 2) / 2 + BORDER_RESOLUTION) * PIXELS_PER_TILE
	]
...
func _ready() -> void:
	...
	GLOBAL_CHUNK_COUNT = Vector2i(
		int(
			ceil((MAP_RESOLUTION.x - BORDER_RESOLUTION * 2) * PIXELS_PER_TILE / GLOBAL_chunk_length)
		),
		int(
			ceil((MAP_RESOLUTION.y - BORDER_RESOLUTION * 2) * PIXELS_PER_TILE / GLOBAL_chunk_length)
		)
	)
	if FLAG_VERBOSE:
	...
	for i in range(data[4].size()):
		if data[4][i] == 3:
			resourceExtents.append(
				[
					data[2][i],
					resource_index[i - biome_index.size()],
					data[3][i],
					data[9][i],
					data[1][i]
				]
...
func _poisson_dd_2d(
	...
	for attempt in max_poisson_attempts_2d:  # Cap number of attempts in case to prevent infinite loop
		...
		var pointLocation: Vector2i = Vector2i(
			randi_range(top_left.x, bottom_right.x), randi_range(top_left.y, bottom_right.y)
		)
		#var global_x = x + Vector2i(1, 1) * int(BORDER_RESOLUTION * PIXELS_PER_TILE)
		var chunk_index: Vector2i = Vector2i(
			int(pointLocation.x / chunkLength), int(pointLocation.y / chunkLength)
		)
		for dx: int in range(-2, 3):
			for dy: int in range(-2, 3):
				var adj_chunk_index: Vector2i = chunk_index + Vector2i(dx, dy)
				if (
					(adj_chunk_index.x >= 0 and adj_chunk_index.y >= 0)
					and (adj_chunk_index.x < chunks.size() and adj_chunk_index.y < chunks[0].size())
				):  # CHECK FOR MAX BOUNDS!
					for point: Vector2 in chunks[adj_chunk_index.x][adj_chunk_index.y]:
						if (
							Vector2i(point.x, point.y).distance_squared_to(pointLocation)
...
func get_data() -> Array:
	...
	scalingFactor = Vector2(
		(MAP_RESOLUTION.x - BORDER_RESOLUTION * 2) * PIXELS_PER_TILE / gen_screen_resolution.x,
		(MAP_RESOLUTION.y - BORDER_RESOLUTION * 2) * PIXELS_PER_TILE / gen_screen_resolution.y
	)
	...
	var arrayToReturn = [
		sqrt(
			(
				(
					((MAP_RESOLUTION.x - BORDER_RESOLUTION * 2) * PIXELS_PER_TILE)
					* ((MAP_RESOLUTION.y - BORDER_RESOLUTION * 2) * PIXELS_PER_TILE)
				)
				/ (number_of_features * PI * sphere_packing_constant)
			)
		),
		features,
		spawn_area,
...
func _generate_points(coverTerrain, resources) -> Array:
	...
	for type in coverTerrain:
		for bounding_area in type[0]:
			...
			var chunk_count: Vector2i = Vector2i(
				int(ceil((bounding_area[1][0] - bounding_area[0][0]) / chunkLen)),
				int(ceil((bounding_area[1][1] - bounding_area[0][1]) / chunkLen))
			)
			...
			for i in type[2]:  #the number of occurences of the type
				...
				var chunk_index = Vector2i(
					int(floor((center.x - topLeft.x) / chunkLen)),
					int(floor((center.y - topLeft.y) / chunkLen))
				)
	...
	for r in resources:
		if FLAG_VERBOSE:
			print("")
		if FLAG_VERBOSE:
			print("GENERATING POINT FOR: ", r[4])
		if FLAG_VERBOSE:
			print("IN RANGE: ")
		if FLAG_VERBOSE:
			print(r[0])
			...
			if r[3] != null:
				...
				for i in r[2]:  #the number of occurences of the type
					...
					if FLAG_VERBOSE:
						print("")
					if FLAG_VERBOSE:
						print("GENERATING " + r[r.size() - 1] + " HERE: ")
					if FLAG_VERBOSE:
						print(center)
					if FLAG_VERBOSE:
						print("")
	...
	if FLAG_VERBOSE:
		print("CHECKING FOR UNPLACED RESOURCES: " + str(unplaced))
	if FLAG_VERBOSE:
		print("")
...
func _generate_mesh(baseTerrain: Array, coverTerrain: Array, resources: Array):  #
	...
	for row: int in MAP_RESOLUTION.y:
		...
		for col: int in MAP_RESOLUTION.x:
			...
			if (
				col < (BORDER_RESOLUTION - BEACH_RESOLUTION)
				or row < BORDER_RESOLUTION - BEACH_RESOLUTION
				or col > (MAP_RESOLUTION.x - (BORDER_RESOLUTION) + BEACH_RESOLUTION)
				or row > (MAP_RESOLUTION.y - BORDER_RESOLUTION + BEACH_RESOLUTION)
			):
				var color = 2  #WATER
			...
			if (
				col <= (BORDER_RESOLUTION)
				or row <= BORDER_RESOLUTION
				or col >= (MAP_RESOLUTION.x - (BORDER_RESOLUTION))
				or row >= (MAP_RESOLUTION.y - BORDER_RESOLUTION)
			):
			...
			for type in baseTerrain:
				...
				for area in type[0]:
					if col * PIXELS_PER_TILE >= area[0][0] and col * PIXELS_PER_TILE <= area[1][0]:
						if (
							row * PIXELS_PER_TILE >= area[0][1]
							and row * PIXELS_PER_TILE <= area[1][1]
						):
							IN = true
				if IN:
	...
	for blob in coverTerrain:
		...
		if col_start < BORDER_RESOLUTION:
			col_start = BORDER_RESOLUTION + 1
		if col_end > MAP_RESOLUTION.x - BORDER_RESOLUTION:
			col_end = MAP_RESOLUTION.x - BORDER_RESOLUTION
		if row_start < BORDER_RESOLUTION:
			row_start = BORDER_RESOLUTION + 1
		if row_end > MAP_RESOLUTION.y - BORDER_RESOLUTION:
			row_end = MAP_RESOLUTION.y - BORDER_RESOLUTION
	...
	for r in resources:
		...
		var quad = null
		if (
			rX <= (MAP_RESOLUTION.x / 2) / 2
			or (rX >= (MAP_RESOLUTION.x / 2) and rX <= (MAP_RESOLUTION.x * 3 / 4))
		):
		...
		while surroundingWater >= 2:  # ADD A PUSHING SIMILAR TO THIS FOR BIOME SPECIFIC RESOURCES
			...
			if FLAG_VERBOSE:
				print(str(rX) + ", " + str(rY))
			if FLAG_VERBOSE:
				print("HAS " + str(surroundingWater) + " PIXELS")
...
func generate_blob(
	center: Vector2, base_radius: float, num_points: int, roughness: float
) -> PackedVector2Array:
...
func roughenMap(base_map: Array) -> Array:
	...
	for y in actual_height:
		...
		for x in actual_width:
			if (
				x < BORDER_RESOLUTION
				or x >= actual_width - BORDER_RESOLUTION
				or y < BORDER_RESOLUTION
				or y >= actual_height - BORDER_RESOLUTION
			):
